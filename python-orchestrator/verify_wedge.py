...
class ExperimentRunner:
    ...
    def run_seed(self, seed: int):
        ...
        for _ in range(max_retries):
            ...
            # 3. DIAGNOSTIC MODE
            if mode == "DIAGNOSTIC":
                # Evaluate Previous Step (if we are past index 0)
                ...
                if diagnostic_index >= len(diagnostic_queue):
                     ...
                     outcome = "UNSAT_WEDGED" if wedged_count == 4 else "ESCAPABLE_POLICY_FAIL"
                     print(f"CONCLUSION: {outcome}")
                     
                     return {
                        "seed": seed, "success": False, "attempts": attempts,
                        "status": outcome, "diagnostic_data": diagnostic_results
                     }
            ...
            # 1. FORCED RETREAT (Safety Mechanism)
            if mode == "REROUTE" and reroute_stage == "OFFSET":
                safe_offset_x = obstacle_plane_x - 1.2
                # DEBUG PRINT
...
if __name__ == "__main__":
    print("--- Running WEDGE VERIFICATION: Seeds 2000, 2001, 2003 ---")
    
    seeds = [2000, 2002, 2003] # Include 2002 as a control (it should escape via Forced Retreat)
    # seeds = [2000, 2001, 2003] # The black holes
    
    runner = ExperimentRunner(ExperimentConfig(mode="adaptive", num_seeds=len(seeds)))
    
    results = []
    
    for seed in seeds:
        print(f"\n[Verify] Starting Seed {seed}...")
        res = runner.run_seed(seed)
        results.append(res)
        
        status = res.get('status', 'FAIL' if not res['success'] else 'SUCCESS')
        print(f"SEED {seed}: {status} ({res['attempts']} steps)")
            
    # Print Summary of Diagnoses
    print("\n--- DIAGNOSTIC SUMMARY ---")
    for r in results:
        print(f"Seed {r['seed']}: {r.get('status', 'UNKNOWN')}")
        if 'diagnostic_data' in r:
            print(f"  Diagnostic: {r['diagnostic_data']}")
